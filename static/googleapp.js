...
function displayBooks(books, containerId, clearContainer = false, isSaved = false, mode = 'default') {
    const container = document.getElementById(containerId);
    if (!container) {
        console.error('Container nicht gefunden:', containerId);
        return;
    ...
    books.forEach(book => {
        ...
        const bookElement = createBookElement(title, authors, thumbnail, bookId, isSaved, mode);
        container.appendChild(bookElement);
    });
}
